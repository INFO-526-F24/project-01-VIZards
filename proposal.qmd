...
In the following project proposal, we analyzed three datasets—Diwali Sales Data, Tucson Police Reported Crime Analysis, and Carbon Major Emissions—providing an introduction to each dataset, its source, and addressing two research questions for each.
...
#### **1 : How do customer demographics—specifically age group and gender—influence spending behavior during the Diwali sales period?**
...
#### **2 : What role do geographic location and marital status play in shaping spending patterns during the Diwali sales period?**
...
```{r}
#| label: glimpse_diwali_sales_data
#| message: false
...
After thoroughly analyzing the three datasets, we have decided to move forward with the **Tucson Police Reported Crime Analysis dataset** for the remainder of the project. This decision was made based on the depth of insights that can be derived from the dataset, its relevance to the research questions, and the potential impact of the findings on understanding crime patterns in Tucson. By focusing on this dataset, we aim to conduct a comprehensive analysis and effectively address the research questions that align with our project's objectives.
